...
import type { Cache } from 'cache-manager'; // Виправлено: додано type
...
export class OrdersService {
  ...
  async findAll(query: any, userId: number, userRole: string) {
  ...
  async findOne(id: number, userId: number, userRole: string): Promise<Order> {
  ...
  async updateStatus(id: number, status: OrderStatus): Promise<Order> {
    ...
    // Виправлено: додано явну типізацію Record для об'єкта
  ...
  private async clearProductsCache() {
    // Виправлено: обхід помилки типізації Cache Manager
    const anyCache = this.cacheManager as any;
    if (anyCache.store && typeof anyCache.store.keys === 'function') {
      const keys: string[] = await anyCache.store.keys('products:*');
